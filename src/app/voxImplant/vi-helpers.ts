...
function getDVideo(containerW: number, containerH: number) {
...
export function setVideoSectionWidth() {
  ...
  const videoSection: HTMLElement = document.querySelector('.conf__video-section') as HTMLElement;
  // @ts-ignore
  ...
  const { Nx, Ny, targetW, targetH } = scaleSelector(N, containerW, containerH);
  ...
  allVideo.forEach((el) => {
    ...
    if (video) {
      ...
      setTimeout(() => {
        if (video) video.style.objectFit = getDVideo(containerW, containerH) !== dVideo ? 'cover' : 'contain';
      }, 1000);
    }
  });
...
function scaleSelector(N: number, containerW: number, containerH: number) {
  if (N === 1) {
...
function isScaleLessMinimum(scale: scaleSelectorResult, containerW: number, containerH: number) {
  const maxCols = containerW > containerH ? 5 : 3;
  return scale.Nx > maxCols;
}

function rescaleSelectorMini(N: number, containerW: number, containerH: number): scaleSelectorResult {
...
function scaleSelectorW(N: number, containerW: number, containerH: number): scaleSelectorResult {
...
function scaleSelectorH(N: number, containerW: number, containerH: number): scaleSelectorResult {
  const sqrCont = containerW * containerH;
  let sqr = {};
  for (let i = 1; i <= N; i++) {
